...
def _repo_config_signatures_path() -> str | None:
    """Dev: config/signatures.json (ou .dist) na raiz do repo."""
    if getattr(sys, "frozen", False):
        return None
    here = os.path.dirname(os.path.abspath(__file__))
    root = os.path.dirname(here)
    for name in ("signatures.json", "signatures.dist.json"):
...
def _candidate_signature_paths() -> list[str]:
    ...
    conf = _repo_config_signatures_path()
    if conf and conf not in paths:
        paths.append(conf)
    return paths
...
def _merge_external_scanner_sigs(data: dict) -> int:
    ...
    try:
        from . import external_scanner as ext
    except ImportError:
        return 0
